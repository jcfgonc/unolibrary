...
public class RawConsoleInput {
	 ...
	 *
	 * @param wait <code>true</code> to wait until an input character is available, <code>false</code> to return immediately if no character is
	 *             available.
	 * @return -2 if <code>wait</code> is <code>false</code> and no character is available. -1 on EOF. Otherwise an Unicode character code within the
	 *         range 0 to 0xFFFF.
	 */
	 ...
	 *
	 * <p>
	 * On Unix this method switches the console back to echo mode. read() leaves the console in non-echo mode.
	 */
